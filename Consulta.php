...
if ($ConsultaInnerJoin->statusConsulta == 'Finalizada') {
    ...
    foreach ($tratamentos as $tratamento) {
        ...
        if ($tratamento->nomeProcedimento == 'Protese'){
            $resultados .= '<tr>
                        <td><a href="pesquisarProtese.php?idConsulta='.$_GET["id"].'&idProcedimento=2&prontuario='.$ConsultaInnerJoin->prontuario.'" style = "text-decoration:none;color:red">' .$tratamento->nomeProcedimento . '</a></td>
                        </tr>';
        }else{
            $resultados .= '<tr>
